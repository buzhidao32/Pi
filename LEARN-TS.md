...
### 4.6 同步 vs 异步：该用哪个？（M6 关键）
> **异步不是"更高级"，同步不是"更落后"，看场景。**

| | 同步（readFileSync） | 异步（readFile） |
|---|---|---|
| 行为 | 卡住等它完成，不放控制权 | 让出控制权，去处理别的事 |
| 适合 | 快、时序敏感 | 慢、长时间（大文件、网络） |

- **时序敏感场景**（如启动时读配置再读输入）：用同步，避免事件循环"插队"。
- **大文件/网络**：用异步，避免卡死程序。
- **事件循环**（单厨餐厅比喻）：JS 单线程，`await` 会"放下手里活去接别的单"。事件循环趁机处理了 stdin 管道数据 → 之后再读就 EOF 了。
- **正确顺序**：先 `await` 异步读取完成，再创建 readline，就不会丢输入。

### 4.6.1 时序问题到底是什么
> **时序 = 做事的先后顺序和时间点。** "时序敏感" = 代码是否正常**很依赖**每个动作发生的先后。
> 同步代码一行行老实执行 → 时序可控，不会被打断 → 基本无时序问题。
> 异步 `await` 会"松手"让事件循环插队 → 时机不可控 → 易出时序问题。
> 例（我们的 bug）：`readline 监听 stdin` → `await 读文件`（松手，事件循环消费 stdin）→ 回来读 stdin 已 EOF。代码没错，错在时机。改同步 `readFileSync` 锁死时序解决。
> **口诀：时序敏感的操作（读输入、初始化）优先用同步，锁死时机。**

### 4.7 stdin / 管道 / EOF（水管比喻）
> **stdin = 给程序送数据的入口水管。** 终端打字、管道（`printf x \| cmd`）、文件重定向都是往 stdin 灌水。`rl.question()` = 从水管接水。EOF = 水管关死（没水了）。

### 4.8 JSON.parse 不是"专转数组"
> `JSON.parse` = `json.loads`，把 JSON 文本转成 JS 数据，**转成啥看内容**：`'[1,2,3]'`→数组，`'{"a":1}'`→对象。所以 parse 后常要 `Array.isArray(x)` 确认类型。
> 记住：**parse 是听话的翻译官，原文是数组就译成数组，原文是对象就译成对象。**

### 4.9 `JSON.stringify(x, null, 2)` 的三参数
> 参数1=要转的数据，参数2=替换器（固定 null），参数3=缩进格数（等同 Python `json.dumps(x, indent=2)`）。
